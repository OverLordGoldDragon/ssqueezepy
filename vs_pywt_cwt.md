...
def cwt_step(i, x, data, scales, out, int_psi, size_scale0, precision):
    ...
    #
    #      j will essentially be a resampled `x`(xval);
    #      `linrange = max(x) - min(x)`, and `step = x[1] - x[0]`, where
    #      `x` is evenly-spaced.
    #      `int_psi` is generated by inputting `x`, so it has same number of samples.
    #      j[-1] == max(j) thus shouldn't exceed `int_psi.size`, but we check
    #      for it in case (... float error?), and drop anything in excess,
    #      as it cannot index `int_psi`.
    #
